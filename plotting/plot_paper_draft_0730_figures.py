...
The EEG figures are built from subject-level NPY outputs under data/results
and data/results2. Report-level summary values are used only for the merged
...
OUT_DIR = Path("figures/paper_draft_0730")
...
def savefig(name):
    ...
    pad_inches = 0.0 if name == "figure5_rww_bw_stability_results" else 0.1
    for ext in ("png", "pdf"):
...
def load_cohort1(feature, transform=True):
    """Pre/post dataset from data/results2, displayed as Cohort 2."""
    root = Path("data/results2")
...
def load_cohort2(feature, transform=True):
    """Longitudinal first-wave dataset from data/results, displayed as Cohort 1."""
    root = Path("data/results")
    visit_counts = {10: 3, 1: 5, 2: 6, 3: 3, 4: 3, 5: 3, 6: 5, 7: 5, 8: 4, 9: 4}
    subjects = [next(root.glob(f"{sid}_*")) for sid in visit_counts]
    ...
    fma_i = 0
    for subject in subjects:
        sid = int(subject.name.split("_")[0])
...
def plot_figure1_criticality():
    ...
    savefig("figure1_criticality_results")


def plot_figure2_fei():
    info = get_info()
    ...
    add_trajectory_caption(fig, handles)
    savefig("figure2_fei_results")


def plot_figure3_complexity():
    info = get_info()
    ...
    add_trajectory_caption(fig, handles)
    savefig("figure3_complexity_results_with_cohort1")


def plot_figure4_behavior():
    ...
    savefig("figure4_behavior_results")


def plot_figure5_rww():
    ...
    savefig("figure5_rww_results")


def plot_figure5_rww_bw():
    ...
    savefig("figure5_rww_bw_results")


def plot_figure5_rww_bw_stability():
    fit_path = Path("data/model/dmf_rww_ei_jax_gpu_bw_fc_only_20260824_bw_forest90_fixed_sigma_i0.csv")
    sig_path = Path("data/model/model_signal_fei_compare_bw_rww_20260825_bw_forest90_fixed_sigma_i0_balanced.csv")
    ...
    savefig("figure5_rww_bw_stability_results")

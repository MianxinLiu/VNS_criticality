...
import plot_paper_draft_0730_figures as source


def main():
    source.setup_style()
    tree = ast.parse(Path(source.__file__).read_text())
    function = next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == 'plot_figure4_behavior')
    ...
    stem = Path('figures/paper_draft_0730/supplementary_figure4_bc_fdr')
    ...
    fig.text(.18, .055, 'Benjamini–Hochberg FDR correction applied separately to the 18 tests in each panel. Bold: p < 0.05.\n'
             ...
             'B: model estimates and raw p values retained at the precision stored in the original Figure 4 script.', fontsize=10, linespacing=1.6)
    for suffix in ['.png', '.pdf']:
        fig.savefig(stem.with_suffix(suffix), dpi=300, bbox_inches='tight')
    plt.close(fig)
    stem.with_suffix('.md').write_text('''Supplementary Figure. Raw and FDR-corrected p values for Figure 4B and C.
